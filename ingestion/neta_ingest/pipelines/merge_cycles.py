...
def _prune_non_current(s) -> int:
    """Delete persons whose latest term predates the current LS cycle (prior-cycle-only winners).

    These are people who won an earlier election but are not sitting now; they were ingested only to
    backfill incumbents' history and shouldn't appear as current legislators.
    """
    latest = s.execute(
